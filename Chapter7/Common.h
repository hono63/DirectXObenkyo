...
public:
	...
	static void MakeBuffResource2(ID3D12Device* _dev, UINT64 buffsize, ID3D12Resource** resource)
	{
	...
	void MakeLayout()
	{
		...
		// テクスチャ uv情報
		mLayout[1].SemanticName = "TEXCORD"; // テクスチャであることを意味する
		mLayout[1].SemanticIndex = 0;
		mLayout[1].Format = DXGI_FORMAT_R32G32_FLOAT; // 32bit x 2 のデータ
